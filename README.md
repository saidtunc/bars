...
| Your user in the `docker` group | run `./bars fix-docker` — see [security note](#security-model) |
...
# 1. Configure — set a unique NODE_ID and a real AUTH_SECRET_KEY
cp .env.example .env && $EDITOR .env

# 2. One-time: allow your user to reach the Docker daemon (prompts for confirmation)
./bars fix-docker      # then log out/in, or: newgrp docker

# 3. Bring up backend (:8000), frontend (:3000) and the host agent (:8001)
./bars start

# 4. Seed the methodology library (checklists, flows, variables)
...
| `AUTH_SECRET_KEY` | `ptf-dev-change-me` | **Change this.** Signs session tokens. |
...
| `HOST_AGENT_TOKEN` | *(empty)* | Shared secret for the host agent. Auto-generated by `bars start`; the agent won't run without it. |
...
`./bars` bootstraps its own venv on first run — no install step.
